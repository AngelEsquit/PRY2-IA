...
La Figura \ref{fig:prim_maze} muestra un laberinto generado con Prim, donde se observa una estructura más lineal con ciertas preferencias direccionales. La Figura \ref{fig:kruskal_maze} presenta un laberinto de Kruskal con distribución más equilibrada. La Figura \ref{fig:generation_comparison} es una animación GIF que visualiza paso a paso cómo ambos algoritmos construyen sus respectivas estructuras.
...
Las siguientes figuras muestran tres escenarios distintos visualizando las diferencias entre algoritmos:

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../reports/problema3/prim/scenarios/scenario_01/02_comparison.png}
\caption{Escenario 1 (Prim)}
\label{fig:scenario_example_01}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../reports/problema3/prim/scenarios/scenario_10/02_comparison.png}
\caption{Escenario 10 (Prim)}
\label{fig:scenario_example_10}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{../reports/problema3/prim/scenarios/scenario_25/02_comparison.png}
\caption{Escenario 25 (Prim)}
\label{fig:scenario_example_25}
\end{figure}
...
\hline
\textbf{Algoritmo} & \textbf{Prim} & \textbf{Kruskal} & \textbf{$\Delta$ (Prim-Kruskal)} \\
\hline
A* & 1.320 & 1.480 & -0.160 \\
\hline
...
class SearchResult:
    ...
    explored_count: int           # Numero de nodos expandidos
    explored_nodes: Set[Cell]     # Conjunto de explorados
    explored_order: List[Cell]    # Orden de exploracion
...
\begin{lstlisting}
import time
t0 = time.perf_counter()    # Precision de nanosegundos
# ... algoritmo ...
...
\begin{itemize}
    ...
    \item Rankings tienen varianza moderada ($\sigma \approx 0.3$--$0.5$)
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Estadísticas cuantitativas del proyecto}
\begin{tabular}{|l|r|}
\hline
\textbf{Concepto} & \textbf{Cantidad} \\
\hline
\multicolumn{2}{|l|}{\textbf{Generadores de Laberintos}} \\
\hline
Generadores implementados & 2 (Prim, Kruskal) \\
\hline
\multicolumn{2}{|l|}{\textbf{Algoritmos de Búsqueda}} \\
\hline
Algoritmos implementados & 4 (BFS, DFS, UCS, A*) \\
\hline
Heurísticas para A* & 1 (Manhattan) \\
\hline
\multicolumn{2}{|l|}{\textbf{Laberintos Generados}} \\
\hline
Problema 1 (demo) & 1 × (60×80) \\
\hline
Problema 2 (resolución) & 8 (2 generadores × 4 algoritmos) \\
\hline
Problema 3 (comparación) & 50 (25 × 2 generadores) \\
\hline
Total laberintos & 59 \\
\hline
\multicolumn{2}{|l|}{\textbf{Experimentos Ejecutados}} \\
\hline
Búsquedas totales & 200 (25 × 2 × 4) \\
\hline
Datos de métricas & 1600 puntos \\
\hline
\multicolumn{2}{|l|}{\textbf{Artefactos Generados}} \\
\hline
Imágenes PNG & 129 \\
\hline
Animaciones GIF & 9 \\
\hline
Archivos CSV & 2 (200 registros totales) \\
\hline
Archivos de ranking TXT & 2 \\
\hline
\multicolumn{2}{|l|}{\textbf{Código}} \\
\hline
Módulos Python & 6 \\
\hline
...
\subsection{Compliance con Requisitos}

\begin{table}[H]
\centering
\caption{Checklist de requisitos del Proyecto 2}
\begin{tabular}{|p{6cm}|c|}
\hline
\textbf{Requisito} & \textbf{Estado} \\
\hline
\textit{Problema 1: Generación de Laberintos} & \\
Implementar $\geq 2$ algoritmos MST & \checkmark \\
Visualizar construcción laberintos & \checkmark \\
Comparar algoritmos visualmente & \checkmark \\
Animaciones de construcción & \checkmark \\
\hline
\textit{Problema 2: Resolución de Laberintos} & \\
Laberinto 60×80 & \checkmark \\
Rutas (1,1) a (60,80) & \checkmark \\
Implementar BFS & \checkmark \\
Implementar DFS & \checkmark \\
Implementar UCS/Dijkstra & \checkmark \\
Implementar A* & \checkmark \\
Mostrar celdas exploradas & \checkmark \\
Mostrar camino encontrado & \checkmark \\
Estadísticas (longitud, explorados) & \checkmark \\
Usar ambos generadores & \checkmark \\
Animaciones de resolución & \checkmark \\
\hline
\textit{Problema 3: Comparación} & \\
Grid 45×55 & \checkmark \\
25 escenarios aleatorios & \checkmark \\
Restricción Manhattan $\geq 10$ & \checkmark \\
Comparar 4 algoritmos & \checkmark \\
Tabla de métricas & \checkmark \\
Ranking por escenario & \checkmark \\
Tabla resumen promedio & \checkmark \\
Visualizaciones escenarios & \checkmark \\
\hline
\textit{Reporte} & \\
Explicación algoritmos MST & \checkmark \\
Explicación algoritmos búsqueda & \checkmark \\
Descripción técnica implementación & \checkmark \\
Arquitectura y clases & \checkmark \\
Visualizaciones Problema 1 & \checkmark \\
Visualizaciones Problema 2 & \checkmark \\
Visualizaciones Problema 3 & \checkmark \\
Tablas comparativas & \checkmark \\
Conclusiones & \checkmark \\
Sin código en reporte & \checkmark \\
Formato PDF/LaTeX & \checkmark \\
\hline
\end{tabular}
\end{table}

\subsection{Configuraciones Computacionales}

\begin{table}[H]
\centering
\caption{Parámetros computacionales del sistema}
\begin{tabular}{|l|l|}
\hline
\textbf{Parámetro} & \textbf{Valor} \\
\hline
Precisión de tiempo & \texttt{time.perf\_counter()} (nanosegundos) \\
Frames por animación & 140 \\
FPS animaciones & 12 \\
Resolución imágenes & 180 DPI \\
Compresión PNG & Máxima \\
Dimensiones Problema 1 & 60×80 celdas \\
Dimensiones Problema 2 & 60×80 celdas \\
Dimensiones Problema 3 & 45×55 celdas \\
Distancia Manhattan mínima & 10 unidades \\
Escenarios de comparación & 25 \\
\hline
\end{tabular}
\end{table}
...
|-- docs/
|   `-- Proyecto2_Reporte.tex    # Este reporte (LaTeX)
|-- src/                         # Codigo fuente
|   |-- __init__.py
|   |-- main.py                  # Punto de entrada CLI
|   |-- menu.py                  # Interfaz TUI
|   |-- runner.py                # Orquestacion
|   |-- maze/
|   |   |-- grid.py              # Representacion de laberintos
|   |   `-- generators.py        # Prim y Kruskal
|   |-- search/
|   |   `-- algorithms.py        # BFS, DFS, UCS, A*
|   |-- visualization/
|   |   `-- plotting.py          # Generacion de graficos
|   `-- experiments/
|       `-- compare.py           # Comparacion y ranking
|-- scripts/
|   `-- run_all.py               # Script de ejecucion
|-- tests/
|   |-- test_baseline.py         # Pruebas unitarias
|   `-- test_compare.py          # Pruebas de comparacion
`-- reports/
    |-- problema1/               # Salida Problema 1
    |-- problema2/               # Salida Problema 2
    `-- problema3/               # Salida Problema 3
        `-- kruskal/
            |-- comparison.csv
            |-- ranking_summary.txt
            `-- scenarios/ (25 directorios)
\end{verbatim}
...
\begin{table}[H]
\centering
\caption{Librerías Python utilizadas}
\begin{tabular}{|l|l|l|}
\hline
\textbf{Librería} & \textbf{Versión} & \textbf{Uso} \\
\hline
numpy & 1.26.4 & Operaciones numéricas \\
pandas & 2.2.2 & Análisis de datos CSV \\
matplotlib & 3.9.2 & Generación de gráficos \\
scikit-learn & 1.5.2 & Utilidades estadísticas \\
PIL/Pillow & (implícita) & Manejo de imágenes GIF \\
...
\begin{enumerate}
    \item Ejecutar con Python 3.10+ en Linux/macOS/Windows
    \item Instalar dependencias: \texttt{pip install -r requirements.txt}
    \item Ejecutar comparación: \texttt{python -m src.main compare --k 25}
    \item Resultados en \texttt{reports/problema3/}
    \item Semillas utilizadas: hardcodeadas para reproducibilidad
\end{enumerate}

Notas:
\begin{itemize}
    \item Tiempos pueden variar ±5\% según carga del sistema
    \item Rankings son determinísticos dados los mismos laberintos
    \item GIFs pueden tener variaciones menores de compresión
\end{itemize}
